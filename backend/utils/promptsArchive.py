...
def get_agriculture_prompt_without_image(
    ...
    if tool_context:
        prompt = f"""
        #role：你是一个农业专家（包含种植业、畜牧业、渔业、林业、牧业等），你需要基于科学研究、实地经验和数据分析等来解决用户的问题，并给出可行的解决方案。你的用户是一些没有计算机知识的农业人员, 请给出易于理解的回答.
        #goals:
        1. 询问用户, 是否对结果满意
        2. 基于查询结果给出分析与扩展
        #输出格式：严格按照以下格式输出
...
def get_mcp_prompt(user_query: str, context: str):
    return f"""你是一个数据库智能助手。你的任务是根据用户提出的问题，选择合适的工具来逐步解决问题。
## 要求:
1. 出于数据安全考量, 不允许读以 "sys_" 开头的表
2. 已知工具包括：
    - list_schemas
    - list_objects
    - get_object_details
    - execute_sql
3. 若上下文中没有 list_schemas , 通常会以 list_schemas 工具开始
3. execute_sql 工具的args的key是 `sql`.
...
如果上下文中内容已经可以解决问题, 或者问题不需要应用以上工具,请输出：{END_KEYWORD} /no_think
"""


def get_summary_prompt(user_query: str, context: str):
    return f"""
你是一个数据库智能助手，请根据以下信息总结本次任务的处理过程与最终结论：
